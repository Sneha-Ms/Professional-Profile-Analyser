...
def secSkillMatch(final_words):    
    ...
    for w in final_words:
        ...
        if w in secondary_list:
            sec_skill_present.append(w)
    return sec_skill_present
...
class resume:
    ...
    def getExp(self):
        return self.exp
    def getName(self):
        t=re.sub(r"/run/media/sneha/OS/Users/sneha/Resumes/","",self.path)
        return t
...
def allToExcel():
    ...
    for path, subdirs, files in os.walk(r"/run/media/sneha/OS/Users/sneha/Resumes"): 
        for name in files:
    ...
    try:
            messagebox.showinfo("Successfully Completed", "All Resume's data has been successfully exported to allResumeData.csv")
    except:
...
def recommend():
    import ast
    try:
        user_description=text_entry.get()
        if user_description=='':
            messagebox.showerror("No query entered", "Please enter a query into the query box to get recommendations")
            return None
    ...
    with open('Recommended.csv','w',) as f:
        ...
        for data in csv_file:
            if(data.__len__()==0):
                ...
                skill_list=ast.literal_eval(data[2])
                if listMatch(skill_list,job_description,60):
                    thewriter.writerow([sr+1, data[1],data[2],data[3],data[4],data[5]])
                    sr+=1
        
    try:
        messagebox.showinfo("Successfully Completed", "Recommended resumes's data has been successfully exported to recommended.csv")
    except:
...
def recommendInGUI():
    try:
        ...
        messagebox.showerror("Unsuccesful!", "Resume data has not been already made, kindly make resume data first by clicking Compile Data button")
...
window=Tk()
window.title("Professional-Profile Analyser")
window.configure(background="black")
photo1=PhotoImage(file='logo.gif')
Label (window, image=photo1, bg="black") .grid(row=0,column=0,sticky=E)
Label (window, text="What would you like to do?:", bg="black",fg="white",font="none 17 bold") .grid(row=1,column=0,sticky=W)

Label (window, text="Option 1: Travesrse all resumes and compile data into Excel File:", bg="black",fg="white",font="none 12 bold") .grid(row=4,column=0,sticky=W)
Button(window,text='Compile Data',width=11,command=allToExcel).grid(row=5,column=0,sticky=W)

Label (window, text="", bg="black",fg="white",font="none 12 bold") .grid(row=6,column=0,sticky=W)
# Label (window, text="", bg="black",fg="white",font="none 12 bold") .grid(row=7,column=0,sticky=W)

Label (window, text="Option 2: Recommend Resumes based on the query below:", bg="black",fg="white",font="none 12 bold") .grid(row=7,column=0,sticky=W)
text_entry=Entry(window,width=70,bg='white')
text_entry.grid(row=8,column=0,sticky=W)
Button(window,text='Recommend',width=11,command=recommendInGUI).grid(row=9,column=0,sticky=W)

Label (window, text="", bg="black",fg="white",font="none 12 bold") .grid(row=10,column=0,sticky=W)

Label (window, text="Option 3: Exit the program:", bg="black",fg="white",font="none 12 bold") .grid(row=11,column=0,sticky=W)
Button(window,text='Exit',width=11,command=exitGUI).grid(row=12,column=0,sticky=W)
